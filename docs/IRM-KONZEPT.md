...
| `/scheduling/calendar` zeigt `Application error: client-side exception` | Backend liefert `plannedDate` als ISO-String (`2026-04-14T00:00:00.000Z`); Frontend baute `${plannedDate}T${startTime}` → ungültiges Datum → `toISOString()` RangeError | `scheduling-calendar.tsx`: `o.plannedDate!.slice(0, 10)` für Date-only Extraktion (analog für `absence.startDate`) |
| HTTP 400 bei Kalender/Berichte mit `from`/`to` und `limit > 100` | Alle Query-DTOs hatten `@Max(100)`, Frontend sendet `limit=500` (Kalender) / `limit=200` (Wizards) → `limit must not be greater than 100` | Alle 10 Query-DTOs auf `@Max(1000)` erhöht (`query-work-orders`, `-properties`, `-staff`, `-equipment`, `-activity-types`, `-absences`, `-customers`, `-formulas`, `-route-sheets`, `-my-orders`) |
| FullCalendar: `Your license key is invalid` unten am Kalender | `@fullcalendar/resource-timegrid` ist ein **Premium**-Plugin — ohne Lizenz erscheint der Hinweis | `resource-timegrid` + `resource` aus `package.json` entfernt, Tages-Ansicht nutzt `timeGridDay` statt `resourceTimeGridDay` |
| Immobilien-Detail: Badge zeigt „Einheiten (24)", aber nur 5 Zeilen | Seed hatte `units_count=24` gesetzt ohne entsprechende `property_units`-Zeilen | SQL-Resync: `UPDATE properties SET units_count = (SELECT COUNT(*) FROM property_units WHERE property_id = properties.id);` |
